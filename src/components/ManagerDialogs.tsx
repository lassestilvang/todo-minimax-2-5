...
interface SettingsManagerDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export function SettingsManagerDialog({ isOpen, onClose }: SettingsManagerDialogProps) {
  const { showToast } = useToast();
  const [busy, setBusy] = useState(false);
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          exit={{ opacity: 0, scale: 0.95, y: 10 }}
          transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}
          className="relative z-50 w-full max-w-sm rounded-xl border bg-card dark:bg-[#141415] shadow-2xl p-6"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold font-[family-name:var(--font-heading)]">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Download a backup of all your tasks, lists, and labels.
            </p>
          </div>
        </motion.div>
      </motion.div>
